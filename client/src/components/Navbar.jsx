      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-1 sm:space-x-4">
            <NavLink to="/projectgenerator" active={isActive('/projectgenerator')}>
              Project Ideas
            </NavLink>
            
            {isPremium ? (
              <NavLink to="/premium" active={isActive('/premium')} isPremium={true}>
                Premium ✨
              </NavLink>
            ) : (
              <NavLink to="/premium" active={isActive('/premium')}>
                Premium
              </NavLink>
            )}
            ...
            {isLoading ? (
              ...
              <div className="flex items-center space-x-2">
                <NavButton to="/login" primary={isActive('/login')}>
                  Log In
                </NavButton>
                <NavButton to="/signup" primary={true}>
                  Sign Up
                </NavButton>
              </div>
            )}
...
const NavLink = ({ children, to, active, isPremium }) => (
  <Link
    to={to}
...
const NavButton = ({ children, to, primary }) => (
  <Link to={to}>
    <motion.button
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
        primary
          ...
          : 'bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 text-gray-100'
      }`}
    >
      {children}
    </motion.button>
